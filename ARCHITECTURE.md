...
- **Alerting-i real kanala bağlamaq:** hazırda `on_failure_callback` yalnız task log-una `ALERT:` sətri yazır — yəni kimsə Airflow-a baxmasa, uğursuzluqdan xəbər tutmur. Vaxtım olsaydı bunu Slack/Telegram webhook-una (Airflow-un öz `SlackWebhookNotifier`-i və ya sadə `requests` çağırışı ilə) bağlayardım; mesajda `dag_id`/`task_id`/`run_id` və log linki olardı ki, növbətçi birbaşa uğursuz task-a keçə bilsin. Bir də siqnalları səviyyəyə görə ayırardım — bronze ingestion-un düşməsi (data ümumiyyətlə gəlmir) dərhal bildiriş, tək bir dbt test xəbərdarlığı isə gündəlik yığcam hesabat kimi; üstəlik `sla_miss_callback` ilə "işləyir, amma gecikir" halını ayrıca izləyərdim.
